...
$$ \min C = \sum_{i \in S_G} (a_i PG_i^2 + b_i PG_i + c_i) $$

Subject to:
$$ PG_i - PD_i = \Gamma(V, \theta, z(G), z(B)) \quad (i \in S_B) $$
$$ QG_i - QD_i = \Psi(V, \theta, z(G), z(B)) \quad (i \in S_B) $$
...
$$ PL_{ij} = V_i V_j (z(G)_{ij} \cos \theta_{ij} + z(B)_{ij} \sin \theta_{ij})  \quad (i, j \in S_B) $$

$$- V_i^2 z(G)_{ij}\quad (i, j \in S_B)$$

$$ -\underline{P_{Lij}} \le P_{Lij} \le \overline{P_{Lij}} \quad (i, j \in S_B) $$

where $PG_i$ and $V_{Gi}$ are the control variables for the $i$-th generator representing the active power generation and voltage magnitude, respectively; $a_i, b_i$ and $c_i$ are the generation cost coefficients for $i$-th generator; $P_{Di}$ and $Q_{Di}$ are the active and reactive power demand at bus $i$; $V_i$ and $\theta_i$ are the voltage magnitude and phase angle at bus $i$; $\theta_{ij}$ is the phase angle difference between $i$-th and $j$-th bus; $G_{ij}$ and $B_{ij}$ denote the conductance and susceptance between the $i$-th and $j$-th bus, respectively; $z(\cdot)$ represents the topology change function; $\Gamma(\cdot), \Psi(\cdot)$ are the power flow equations; $P_{Lij}$ represents the active branch power between $i$-th node and $j$-th node; $S_G, S_B$ and $S_K$ are the index set of generators, buses and branches.
...
$$ PG_i - PD_i = e_i \sum_{j \in N(i)} (z(G)_{ij} e_j - z(B)_{ij} f_j) + f_i \sum_{j \in N(i)} (z(G)_{ij} f_j + z(B)_{ij} e_j) $$
$$ QG_i - QD_i = f_i \sum_{j \in N(i)} (z(G)_{ij} e_j - z(B)_{ij} f_j) - e_i \sum_{j \in N(i)} (z(G)_{ij} f_j + z(B)_{ij} e_j) $$
...
$$ \delta_i = PG_i - PD_i - (e_i^2 + f_i^2) z(G)_{ii} $$
$$ \lambda_i = QG_i - QD_i + (e_i^2 + f_i^2) z(B)_{ii} $$
...
$$ \boldsymbol{Y} = \begin{bmatrix} \boldsymbol{e}^{l+1} \\ \boldsymbol{f}^{l+1} \end{bmatrix} = f \left( \begin{bmatrix} \frac{\boldsymbol{\delta} \odot \boldsymbol{\alpha} - \boldsymbol{\lambda} \odot \boldsymbol{\beta}}{\boldsymbol{\alpha} \odot \boldsymbol{\alpha} + \boldsymbol{\beta} \odot \boldsymbol{\beta}} & 0 \\ 0 & \frac{\boldsymbol{\delta} \odot \boldsymbol{\beta} + \boldsymbol{\lambda} \odot \boldsymbol{\alpha}}{\boldsymbol{\alpha} \odot \boldsymbol{\alpha} + \boldsymbol{\beta} \odot \boldsymbol{\beta}} \end{bmatrix} \begin{bmatrix} \boldsymbol{W}_1 \\ \boldsymbol{W}_2 \end{bmatrix} + \begin{bmatrix} \boldsymbol{B}_1 \\ \boldsymbol{B}_2 \end{bmatrix} \right) $$
...
$$ PG_i = \max(\min(PG_i, PG_i^{\max}), PG_i^{\min}) $$
$$ QG_i = \max(\min(QG_i, QG_i^{\max}), QG_i^{\min}) $$
